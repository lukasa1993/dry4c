...
def main():
 p=argparse.ArgumentParser(prog="dry4c");p.add_argument("filters",nargs="*");p.add_argument("--root",default=".");p.add_argument("--min-tokens",type=int,default=30);p.add_argument("--json",action="store_true");p.add_argument("--fail",action="store_true");p.add_argument("--version",action="version",version="%(prog)s "+VERSION);a=p.parse_args();r=Path(a.root).resolve();fs=discover(r,EXTS)
